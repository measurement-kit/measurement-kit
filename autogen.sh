...
gen_headers() {
    ...
    # Note: common/version.hpp is generated by autoconf but we also need it
    # here to put it inside common.hpp, hence create and empty file and then
    # remove such file when done
    touch include/measurement_kit/common/version.hpp
    ...
    for name in $(ls include/measurement_kit/); do
        ...
        for nn in $(cd include/measurement_kit &&                              \
                    find $name -type f -name \*.hpp); do
            echo "#include <measurement_kit/$nn>"                         >> $hh
        done
        echo "#endif"                                                     >> $hh
    done
    rm include/measurement_kit/common/version.hpp
