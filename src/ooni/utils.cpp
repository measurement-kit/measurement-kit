...
ErrorOr<std::string> IPLocation::resolve_country_code(std::string ip,
    ...
    if (gi_country == nullptr) {
        ...
        if (gi_country == nullptr) {
            logger->debug("IPLocation: cannot open geoip country database");
...
ErrorOr<std::string> IPLocation::resolve_city_name(std::string ip,
    ...
    if (gi_city == nullptr) {
        ...
        if (gi_country == nullptr) {
            logger->debug("IPLocation: cannot open geoip city database");
...
ErrorOr<std::string> IPLocation::resolve_asn(std::string ip,
    ...
    if (gi_asn == nullptr) {
        ...
        if (gi_asn == nullptr) {
            logger->debug("IPLocation: cannot open geoip asn database");
