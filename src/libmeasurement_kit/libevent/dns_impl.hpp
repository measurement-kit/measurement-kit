...
static inline std::vector<Answer> build_answers_evdns(
    ...
    if (code != DNS_ERR_NONE) {
        ...
        answer.code = code;
        answer.ttl = ttl;
        answer.type = QueryTypeId::DNS_TYPE_PTR;
        ...
        if (count >= 0 && count <= INT_MAX / size + 1) {
            ...
            for (auto i = 0; i < count; ++i) {
                ...
                if (family == PF_INET) {
                    answer.ipv4 = string;
                    answer.type = QueryTypeId::DNS_TYPE_A;
                } else if (family == PF_INET6) {
                    answer.ipv6 = string;
                    answer.type = QueryTypeId::DNS_TYPE_AAAA;
                }
...
void query_impl(QueryClass dns_class, QueryType dns_type, std::string name,
    ...
    if (dns_class != QueryClassId::DNS_CLASS_IN) {
        evdns_base_free(base, 1);
    ...
    // Allow PTR queries
    if (dns_type == QueryTypeId::DNS_TYPE_PTR) {
        std::string s;
        if ((s = mk::unreverse_ipv4(name)) != "") {
            dns_type = QueryTypeId::DNS_TYPE_REVERSE_A;
            name = s;
        } else if ((s = mk::unreverse_ipv6(name)) != "") {
            dns_type = QueryTypeId::DNS_TYPE_REVERSE_AAAA;
            name = s;
        } else {
            evdns_base_free(base, 1);
    ...
    //
    if (dns_type == QueryTypeId::DNS_TYPE_A) {
    ...
    if (dns_type == QueryTypeId::DNS_TYPE_AAAA) {
    ...
    if (dns_type == QueryTypeId::DNS_TYPE_REVERSE_A) {
    ...
    if (dns_type == QueryTypeId::DNS_TYPE_REVERSE_AAAA) {
