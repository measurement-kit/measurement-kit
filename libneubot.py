...
class Connection(ConnectionBase):
    ...
    def attach(protocol, filenum):
        self = Connection()
        self.context_ = LIBNEUBOT.NeubotConnection_attach(protocol, filenum)
    ...
    def connect(protocol, family, address, port):
        self = Connection()
        self.context_ = LIBNEUBOT.NeubotConnection_connect(protocol, family,
          address, port)
        if not self.context_:
            DIE(1)
        _ctypes.Py_INCREF(self)
    ...
    def connect_hostname(protocol, family, address, port):
        self = Connection()
        self.context_ = LIBNEUBOT.NeubotConnection_connect_hostname(protocol,
          family, address, port)
        if not self.context_:
            DIE(1)
        _ctypes.Py_INCREF(self)
