...
static std::vector<uint8_t> get_invalid() {
    const unsigned char *p = (unsigned char *)"invalid";
    std::vector<uint8_t> invalid{p, p + sizeof (p) / sizeof (p[0]) - 1};
    return invalid;
}
...
TEST_CASE("dns::sendrecv() handles net_socket_create() error") {
    ...
    ErrorOr<Var<socket_t>> maybe_sock =
        dns::send_impl<net_socket_create_fail>(
            "8.8.8.8", "53", get_invalid(), Logger::global()
        );
    REQUIRE(!maybe_sock);
...
TEST_CASE("dns::recv() handles recv() failure") {
    Var<socket_t> sock{new socket_t{2}};
    ErrorOr<std::vector<uint8_t>> maybe_str =
...
send_fail(std::string, std::string, std::vector<uint8_t>, Var<Logger>) {
    return MockedError();
}
...
TEST_CASE("dns::sendrecv() handles send() failure") {
    ...
    reactor->loop_with_initial_event([=]() {
        mk::dns::sendrecv_impl<send_fail>(
            "8.8.8.8", "53", get_invalid(),
            [=](Error error, std::vector<uint8_t> s) {
                REQUIRE(error.code == MockedError().code);
                REQUIRE(s == std::vector<uint8_t>{});
                reactor->break_loop();
            },
...
static ErrorOr<Var<socket_t>>
send_okay(std::string, std::string, std::vector<uint8_t>, Var<Logger>) {
    return NoError();
}
...
static ErrorOr<std::vector<uint8_t>> recv_failure(Var<socket_t>, Var<Logger>) {
    return MockedError();
}
