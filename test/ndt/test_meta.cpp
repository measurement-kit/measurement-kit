...
TEST_CASE("run() deals with read() error") {
    ...
    test_meta::run_impl<fail>(ctx, [](Error err) {
        REQUIRE(err == ReadingTestPrepareError());
    });
}
...
TEST_CASE("run() deals with unexpected message type") {
    ...
    test_meta::run_impl<unexpected>(ctx, [](Error err) {
        REQUIRE(err == NotTestPrepareError());
    });
}
...
TEST_CASE("run() deals with second read() error") {
    Var<Context> ctx(new Context);
    test_meta::run_impl<test_prepare, fail>(ctx, [](Error err) {
        REQUIRE(err == ReadingTestStartError());
    });
}

TEST_CASE("run() deals with unexpected message on second read") {
    Var<Context> ctx(new Context);
    test_meta::run_impl<test_prepare, unexpected>(ctx, [](Error err) {
        REQUIRE(err == NotTestStartError());
    });
}
...
TEST_CASE("run() deals with second format_test_msg() error") {
    ...
    test_meta::run_impl<test_prepare, test_start, success, fail>(ctx, [](Error err) {
        REQUIRE(err == SerializingClientApplicationError());
    });
}
...
TEST_CASE("run() deals with third format_test_msg() error") {
    Var<Context> ctx(new Context);
    ctx->txp.reset(new Emitter);
    test_meta::run_impl<test_prepare, test_start, success, success, fail>(ctx, [](Error err) {
        REQUIRE(err == SerializingFinalMetaError());
    });
}
...
TEST_CASE("run() deals with write error") {
    Var<Context> ctx(new Context);
    ctx->txp.reset(new Emitter);
    test_meta::run_impl<test_prepare, test_start, success, success, success, fail>(ctx, [](Error err) {
        REQUIRE(err == WritingMetaError());
    });
}
...
TEST_CASE("run() deals with third read() error") {
    Var<Context> ctx(new Context);
    ctx->txp.reset(new Emitter);
    test_meta::run_impl<test_prepare, test_start, success, success, success, success, fail>(ctx, [](Error err) {
        REQUIRE(err == ReadingTestFinalizeError());
    });
}
...
TEST_CASE("run() deals with unexpected message on third read") {
    Var<Context> ctx(new Context);
    ctx->txp.reset(new Emitter);
    test_meta::run_impl<test_prepare, test_start, success, success, success, success, unexpected>(ctx, [](Error err) {
        REQUIRE(err == NotTestFinalizeError());
    });
}
